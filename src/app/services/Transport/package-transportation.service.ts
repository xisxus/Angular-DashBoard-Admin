...
export class PackageTransportationService {
  private baseUrl = 'http://localhost:5141/api/Package';  // Adjust base URL
  ...
  getPackageTransportations(): Observable<PackageTransportation[]> {
    return this.http.get<PackageTransportation[]>(this.baseUrl, this.httpOptions);
  }
  ...
  updatePackageTransportation(id: number, pkgTrans: PackageTransportation): Observable<PackageTransportation> {
    return this.http.put<PackageTransportation>(`${this.baseUrl}/${id}`, pkgTrans, this.httpOptions);
  }
