...
export class TransportationService {
  ...
  private apiUrl = 'http://localhost:5141/api/Transportation';
  ...
  createTransport(transport: TransportationInputModel): Observable<TransportationInputModel> {
    return this.http.post<TransportationInputModel>(`${this.apiUrl}/add`, transport, {
  ...
  //     );
  // }
  updateTransportation(id: number, model: TransportationInputModel): Observable<any> {
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      // Server-side error
      errorMessage = `Server-side error: Code ${error.status}, Message: ${error.error?.message || error.message}`;
    }
